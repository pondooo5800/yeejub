...
/**
 * [ Controller File name : Products_types.php ]
 */
class Products_types extends CRUD_Controller
{
	...
	{
		$this->session->unset_userdata($this->Products_types->session_name . '_search_field');
		$this->session->unset_userdata($this->Products_types->session_name . '_value');
	...
	public function search()
	{
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Products_types->session_name . '_search_field' => $search_field, $this->Products_types->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Products_types->session_name . '_search_field');
			$value = $this->session->userdata($this->Products_types->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Products_types->order_field = $field;
			$this->Products_types->order_sort = $sort;
		}
		$results = $this->Products_types->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('products_types/products_types');
		...
		$this->render_view('products_types/products_types/list_view');
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$results = $this->Products_types->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('products_types/products_types/preview_view');
			}
		}
	}
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			$id = $this->Products_types->create($post);
			if ($id != '') {
				$success = TRUE;
				...
				$success = FALSE;
				$message = 'Error : ' . $this->Products_types->error_message;
			}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('products_types/products_types/edit_view');
			}
		}
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			$result = $this->Products_types->update($post);
			if ($result == false) {
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Products_types->delete($post);
			if ($result == false) {
				$message = $this->Products_types->error_message;
				$ok = FALSE;
			} else {
	...
	private function setPreviewFormat($row_data)
	{
		...
		$this->data['record_product_name'] = $data['product_type_name'];
