...
/**
 * [ Controller File name : Orders.php ]
 */
class Orders extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['page_url'] = site_url('orders/orders');
		$this->upload_store_path = './assets/uploads/orders/';
	...
	{
		$this->session->unset_userdata($this->Orders->session_name . '_search_field');
		$this->session->unset_userdata($this->Orders->session_name . '_value');
	...
	public function search()
	{
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Orders->session_name . '_search_field' => $search_field, $this->Orders->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Orders->session_name . '_search_field');
			$value = $this->session->userdata($this->Orders->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Orders->order_field = $field;
			$this->Orders->order_sort = $sort;
		}
		$results = $this->Orders->read($start_row, $per_page);
		...
		$page_url = site_url('orders/orders');
		...
		$this->render_view('orders/orders/list_view');
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$results = $this->Orders->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('orders/orders/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->data['data_id'] = 0;
		$this->data['products_types_option_list'] = $this->Orders->returnOptionList("tb_products_types", "product_type_id", "product_type_name");
		...
		$this->render_view('orders/orders/add_view');
	}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('orders/orders/edit_view');
			}
		}
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if ($upload_error == 0) {
				$result = $this->Orders->update($post);
				if ($result == false) {
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Orders->delete($post);
	...
	private function setFagAllowOrder($value)
	{
		$subject = '';
		switch ($value) {
			case '1':
	...
	private function setPreviewFormat($row_data)
	{
		...
		$encrypt_name = end($arr);
		$filename = $this->Orders->getValueOf('tb_uploads_filename', 'filename', "encrypt_name = '$encrypt_name'", $this->db);
