						...
						<div class="row">
							...
							<div class="col-sm-12">
								<div class="row align-items-center">
									...
									<div class="col-md-2">
										<div class="form-group bmd-form-group">
											<select class="select2-search" id="set_order_by" class="span2" value="{order_by}">
												...
												<option value="product_type|asc">ประเภทสินค้า เก่า - ใหม่</option>
												<option value="product_type|desc">ประเภทสินค้า ใหม่ - เก่า</option>
												<option value="banner_type|asc">แบรนด์สินค้า เก่า - ใหม่</option>
												<option value="banner_type|desc">แบรนด์สินค้า ใหม่ - เก่า</option>
												<option value="product_pro_id|asc">โปรโมชั่น เก่า - ใหม่</option>
												<option value="product_pro_id|desc">โปรโมชั่น ใหม่ - เก่า</option>
									...
									<div class="col-md-2">
										<div class="form-group bmd-form-group">
											<a id="deleteSelected" class="btn btn-danger text-white">ลบรายการ</a>
										</div>
									</div>
								</div>
							</div>
						...
						<table class="table table-striped">
							...
							<tbody>
								<tr parser-repeat="[data_list]" id="row_{record_number}">
									<td style="text-align:center;"><input type="checkbox" class="checkItem" value="{record_product_id}"></td>
									<td style="text-align:center;">{record_number}</td>
									<td class="product" style="text-align:center;">
										<a><img class="img-responsive" style="margin-left: 100%; width:180px; height:180px; object-fit:contain" alt="product" src="{base_url}{product_img1}">{product_code}</a>
									</td>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#deleteSelected").click(function() {
			var frm_action = site_url('products/deleteRows/');
			var ids = [];
			...
			if (ids.length > 0) {
				$.ajax({
					...
					error: function(xhr, status, error) {
						...
						Swal.fire({
							title: 'Error!',
							text: 'เกิดข้อผิดพลาดในการลบรายการที่เลือก',
							icon: 'error',
							confirmButtonText: 'OK'
						});
					}
				...
				Swal.fire({
					title: 'Warning!',
					text: 'โปรดเลือกอย่างน้อยหนึ่งรายการที่จะลบ',
					icon: 'warning',
					confirmButtonText: 'OK'
				});
			}
