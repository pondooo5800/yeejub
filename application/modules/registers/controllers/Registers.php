...
/**
 * [ Controller File name : Registers.php ]
 */
class Registers extends CRUD_Controller
{
	...
	protected function render_view($path)
	{
		...
		$this->parser->parse('template/frontend/registerTemplate', $this->data);
	}
	...
	{
		$this->session->unset_userdata($this->Registers->session_name . '_search_field');
		$this->session->unset_userdata($this->Registers->session_name . '_value');
	...
	public function search()
	{
		if (isset($_POST['submit'])) {
			...
			$arr = array($this->Registers->session_name . '_search_field' => $search_field, $this->Registers->session_name . '_value' => $value);
			$this->session->set_userdata($arr);
		} else {
			$search_field = $this->session->userdata($this->Registers->session_name . '_search_field');
			$value = $this->session->userdata($this->Registers->session_name . '_value');
		}
		...
		if ($order_by != '') {
			...
			$this->Registers->order_field = $field;
			$this->Registers->order_sort = $sort;
		}
		$results = $this->Registers->read($start_row, $per_page);
		$total_row = $results['total_row'];
		$search_row = $results['search_row'];
		$list_data = $this->setDataListFormat($results['list_data'], $start_row);


		$page_url = site_url('registers/registers');
		...
		$this->render_view('registers/registers/list_view');
	}
	...
	public function preview($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			$results = $this->Registers->load($id);
			if (empty($results)) {
				$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				$this->render_view('ci_message/danger');
			} else {
				$this->setPreviewFormat($results);
				$this->render_view('registers/registers/preview_view');
			}
		}
	}
	...
	{
		$this->data['data_id'] = 0;
		$this->render_view('registers/registers/add_view');
	}
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			if ($id != '') {
				...
				$success = FALSE;
				$message = 'Error : ' . $this->Registers->error_message;
			}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('registers/registers/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$product_type_id = ci_decrypt($data['encrypt_product_type_id']);
		if ($product_type_id == '') {
			$error .= '- รหัส product_type_id';
		}
		return $error;
	}
	/**
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			$result = $this->Registers->update($post);
			if ($result == false) {
				$message = $this->Registers->error_message;
				$ok = FALSE;
			} else {
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Registers->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
				'is_successful' => $ok,
	...
	public function del()
	{
		...
		if ($message != '') {
			...
			$result = $this->Registers->delete($post);
			if ($result == false) {
				$message = $this->Registers->error_message;
				$ok = FALSE;
			} else {
	...
	private function setDataListFormat($lists_data, $start_row = 0)
	{
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$filename = $this->Registers->getValueOf('tb_uploads_filename', 'filename', "encrypt_name = '$encrypt_file_name'", $this->db);
	...
	{
		$data = $row_data;
		$pk1 = $data['product_type_id'];
		$this->data['recode_url_encrypt_id'] = urlencode(encrypt($pk1));

		if ($pk1 != '') {
			$pk1 = encrypt($pk1);
		}
		$this->data['encrypt_product_type_id'] = $pk1;
		$this->data['record_product_type_id'] = $data['product_type_id'];
