...
var Popups = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('popups/popups/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('popups/popups/save');
						...
						try
						{
							...
							if(results.is_successful){
								...
								window.setTimeout(function () {
									$(window.location).attr("href", site_url("popups"));
								}, 1300);
							}else{
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('popups/popups/update');
		if(!$('#post_iframe').attr('id')){
			var iframe = $('<iframe name="post_iframe" id="post_iframe" style="display: none"></iframe>');
			$("body").append(iframe);
		}
	...
	confirmDelete: function (ppopupId,  irow){
		$('[name="encrypt_popup_id"]').val(ppopupId);
	...
	deleteRecord: function(){
		var frm_action = site_url('popups/popups/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('popups/popups/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSaveEdit').click(function() {
		return Popups.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Popups.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var ppopupId = $(this).attr('data-popup_id');

		Popups.confirmDelete(ppopupId,  row_num);
	});//click
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('popups/popups/deletepopupImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
