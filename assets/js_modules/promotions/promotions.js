...
var Promotions = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('promotions/promotions/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('promotions/promotions/save');
					...
					if(json_string != ""){
						...
						try
						{
							...
							if(results.is_successful){
								notify('แจ้งเตือน', 'บันทึกข้อมูลหลักเรียบร้อย', 'success', 'center');
								$("#frmUploadDetail :input").attr("disabled", false);
								window.setTimeout(function () {
									$(window.location).attr("href", site_url("promotions"));
								}, 1300);
							}else{
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('promotions/promotions/update');
		if(!$('#post_iframe').attr('id')){
			var iframe = $('<iframe name="post_iframe" id="post_iframe" style="display: none"></iframe>');
			$("body").append(iframe);
		}
	...
	confirmDelete: function (ppromotionId,  irow){
		$('[name="encrypt_promotion_id"]').val(ppromotionId);
	...
	deleteRecord: function(){
		var frm_action = site_url('promotions/promotions/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('promotions/promotions/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#promotion_img1').change(function(){
		var msg = '';
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Promotions.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Promotions.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Promotions.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var ppromotionId = $(this).attr('data-promotion_id');

		Promotions.confirmDelete(ppromotionId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Promotions.deleteRecord();
	});
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('promotions/promotions/deletePromotionImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
