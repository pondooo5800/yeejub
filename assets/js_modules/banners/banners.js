...
var Banners = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('banners/banners/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('banners/banners/save');
						...
						try
						{
							...
							if(results.is_successful){
								...
								window.setTimeout(function () {
									$(window.location).attr("href", site_url("banners"));
								}, 1300);
							}else{
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('banners/banners/update');
		if(!$('#post_iframe').attr('id')){
			var iframe = $('<iframe name="post_iframe" id="post_iframe" style="display: none"></iframe>');
			$("body").append(iframe);
		}
	...
	confirmDelete: function (pbannerId,  irow){
		$('[name="encrypt_banner_id"]').val(pbannerId);
	...
	deleteRecord: function(){
		var frm_action = site_url('banners/banners/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url('banners/banners/index/'+ this.current_page));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Banners.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Banners.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Banners.current_page = Math.abs(param_current_page);
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pbannerId = $(this).attr('data-banner_id');

		Banners.confirmDelete(pbannerId,  row_num);
	});//click
...
$(document).ready(function() {
    ...
    $(document).on('click', '.image-cancel', function() {
		...
		$.ajax({
			method: 'POST',
			url: site_url('banners/banners/deleteBannerImage'),
			dataType: 'json',
			data:fdata,
			success: function (results) {
				console.log(results);
